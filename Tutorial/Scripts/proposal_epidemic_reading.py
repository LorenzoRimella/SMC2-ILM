...
class proposal_epidemic():
    ...
    def proposal_epidemic_0(self, Y_0):    
        ...
        return C_0p, W_0p, tf.ones((self.n_particles))*(self.initial_colonize_prob)/self.gamma
    ...
    def proposal_epidemic_t(self, t, theta, C_tp, Y_t, delta_t):
        ...
        if self.epidemic_type == "3 parameters within-across households and fixed recovery":
            ...
            prob_lambda_within = (1- tf.exp(-delta_t*(infection_rate_from_household  )))*(1-C_tp)
            prob_lambda_across = (1- tf.exp(-delta_t*(infection_rate_across_household)))*(1-C_tp)
        ...
        if self.epidemic_type == "fixed effect, 3 parameters within-across households (all proportions) and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            prob_lambda_within = (1- tf.exp(-delta_t*(infection_rate_from_household  )))*(1-C_tp)
            prob_lambda_across = (1- tf.exp(-delta_t*(infection_rate_across_household+ fixed_effect)))*(1-C_tp)
            # recovery probability
            prob_gamma  = tf.exp(-delta_t*self.gamma)*C_tp
            P_t = prob_lambda + prob_gamma
        ...
        # R_t
        # R_t_num = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, prob_lambda)
        # R_t_den = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, C_tp-prob_gamma)
        # R_t_den = tf.cast(R_t_den==0, dtype = tf.float32)*(R_t_num/self.households_individuals.households_size) + R_t_den

        # R_t_beta_1_num = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, prob_lambda_within)
        # R_t_beta_1_den = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, C_tp-prob_gamma)
        # R_t_beta_1_den = tf.cast(R_t_beta_1_den==0, dtype = tf.float32)*(R_t_beta_1_num/self.households_individuals.households_size) + R_t_beta_1_den

        # R_t_beta_2_num = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, prob_lambda_across)
        # R_t_beta_2_den = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, C_tp-prob_gamma)
        # R_t_beta_2_den = tf.cast(R_t_beta_2_den==0, dtype = tf.float32)*(R_t_beta_2_num/self.households_individuals.households_size) + R_t_beta_2_den

        R_t_num = tf.reduce_sum( prob_lambda, axis = 0 )
        R_t_den = tf.reduce_sum( C_tp-prob_gamma, axis = 0 )

        R_t_beta_1_num = tf.reduce_sum( prob_lambda_within, axis = 0 )
        R_t_beta_1_den = tf.reduce_sum( C_tp-prob_gamma, axis = 0 )

        R_t_beta_2_num = tf.reduce_sum( prob_lambda_across, axis = 0 )
        R_t_beta_2_den = tf.reduce_sum( C_tp-prob_gamma, axis = 0 )

        R_t        = R_t_num/R_t_den
        R_t_beta_1 = R_t_beta_1_num/R_t_beta_1_den
        R_t_beta_2 = R_t_beta_2_num/R_t_beta_2_den

        return C_tp1p, W_0p, R_t, R_t_beta_1, R_t_beta_2