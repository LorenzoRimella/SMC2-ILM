...
# Load the data
########################################################
n_individuals_ndirande          = np.load("UC-model/Data/Input/n_individuals_ndirande.npy")
n_households_ndirande           = np.load("UC-model/Data/Input/n_households_ndirande.npy")
households_longitude_ndirande   = np.load("UC-model/Data/Input/households_longitude_ndirande.npy")
households_latitude_ndirande    = np.load("UC-model/Data/Input/households_latitude_ndirande.npy")
loc_H_I_index_ndirande          = np.load("UC-model/Data/Input/loc_H_I_index_ndirande.npy")
individuals_covariates_ndirande = np.load("UC-model/Data/Input/individuals_covariates_ndirande.npy")

Y_ecoli_ndirande = np.load("UC-model/Data/Input/Y_ecoli_ndirande.npy")
Y_klebs_ndirande = np.load("UC-model/Data/Input/Y_kpneu_ndirande.npy")

theta_ndirande_ecoli   = np.load("UC-model/Data/Parameters/theta_best_ndirande_ecoli.npy") 
theta_ndirande_klebs   = np.load("UC-model/Data/Parameters/theta_best_ndirande_klebs.npy") 

########################################################
# Create the household class
########################################################
households_ndirande = households_individuals(n_individuals_ndirande, n_households_ndirande, 
                                             households_longitude_ndirande, households_latitude_ndirande,
                                             loc_H_I_index_ndirande, 
                                             individuals_covariates_ndirande)
...
hyperparameters_epidemic_ecoli["coef"]                  = hyperparameters_epidemic_coef_dict["eps_3param_gauss_season_coef"]
...
hyperparameters_epidemic_klebs["coef"]                  = hyperparameters_epidemic_coef_dict["3param_exp_prop_season_coef_6"]
...
filtering_ndirande_ecoli = Particle_filter_smoother( households_ndirande,   
                                               n_particles,
                                               theta_ndirande_ecoli,
                                               hyperparameters_epidemic_ecoli,
                                               epidemic_type_ecoli)

filtering_ndirande_klebs = Particle_filter_smoother( households_ndirande,   
                                               n_particles,
                                               theta_ndirande_klebs,
                                               hyperparameters_epidemic_klebs,
                                               epidemic_type_klebs)


Y_ecoli =  Y_ecoli_ndirande
Y_complement_ecoli    = np.empty((households_ndirande.n_individuals - Y_ecoli.shape[0], Y_ecoli.shape[1]))
Y_complement_ecoli[:] = np.NaN
Y_ecoli = np.concatenate((Y_ecoli, Y_complement_ecoli), axis = 0)

Y_klebs =  Y_klebs_ndirande
Y_complement_klebs    = np.empty((households_ndirande.n_individuals - Y_klebs.shape[0], Y_klebs.shape[1]))
Y_complement_klebs[:] = np.NaN
Y_klebs = np.concatenate((Y_klebs, Y_complement_klebs), axis = 0)


file_name_ecoli = "ndirande_filtering_ecoli"     
file_name_klebs = "ndirande_filtering_klebs"   


file_name_ecoli = "ndirande_filtering_ecoli"     
file_name_klebs = "ndirande_filtering_klebs"              

output_ecoli = filtering_ndirande_ecoli.filter(Y_ecoli, max_delta_t_ecoli)
output_klebs = filtering_ndirande_klebs.filter(Y_klebs, max_delta_t_klebs)
