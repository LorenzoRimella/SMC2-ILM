# How to use it:
# 1- write for slurm system: SMC2_run_ndirande.com 
# 2- run: qsub -t 1:29 UC-model/RunEcoli/SMC2_run_ndirande.com
...
hyperparameters_epidemic["translation"]           = 0.55*np.pi 
...
path1 = "Experiments/Data/Output/Ecoli/"

path_check1 = "Experiments/Data/Check/Ecoli/"
...
SMC2_ndirande = SMC2_epidemic(households_ndirande, 
...
Y =  Y_ecoli_ndirande
Y_complement    = np.empty((households_ndirande.n_individuals - Y.shape[0], Y.shape[1]))
Y_complement[:] = np.NaN
...
for sim_num in range(1,5):
    file_name = str(sim_num)+"_ndirande_"+simulation              

    outputfile_name = path_check+file_name+".txt"

    theta_particles_history, p_y_t_given_y_1_to_tm1_history, rejuvenation_time_history = SMC2_ndirande.SMC2(Y, max_delta_t, outputfile_name)
