...
from proposal_epidemic_reading import *
...
class Particle_filter_smoother():
    ...
    def step_0(self, Y_0):
        ...
        C_tp, w_tind, R_0 = self.proposal_epidemic.proposal_epidemic_0(Y_0)
        ...
        return C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_0
    ...
    def step_t(self, t, theta, C_tp, A_t, Y_t, delta_t):
        ...
        C_tp, w_tind, R_t, R_t_beta_1, R_t_beta_2 = self.proposal_epidemic.proposal_epidemic_t( t, theta, tildeC_tm1, Y_t, delta_t)
        # C_tp, w_tind, R_t = self.proposal_epidemic.proposal_epidemic_t( t, theta, tildeC_tm1, Y_t, delta_t)
        ...
        return C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_t_beta_2, R_t_beta_1, R_t
    ...
    def filter(self, Y, max_delta_t):
        ...
        C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp = tf.map_fn(fn = step_0_multi, elems = index, dtype=(tf.float32, tf.int32, tf.float32, tf.float32))
        # C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp = tf.map_fn(fn = step_0_multi, elems = index, dtype=(tf.float32, tf.int32, tf.float32, tf.float32))
        ...
        ancestors = 1#tf.reshape(A_t,                           shape = (C_tp.shape[0], C_tp.shape[2], 1))
        infected  = tf.reshape(tf.reduce_mean(tf.reduce_sum(C_tp, axis = 1), axis = 1), shape = (C_tp.shape[0], 1, 1))
        ...
        # R_t_beta_2    = tf.reshape(tf.reduce_mean(R_tp, axis = 2), shape = (C_tp.shape[0], H_tp.shape[1], 1))
        # R_t_beta_1    = tf.reshape(tf.reduce_mean(R_tp, axis = 2), shape = (C_tp.shape[0], H_tp.shape[1], 1))
        # R_t           = tf.reshape(tf.reduce_mean(R_tp, axis = 2), shape = (C_tp.shape[0], H_tp.shape[1], 1))

        R_t_beta_2    = tf.reshape(tf.reduce_mean(R_tp, axis = 1), shape = (C_tp.shape[0], 1))
        R_t_beta_1    = tf.reshape(tf.reduce_mean(R_tp, axis = 1), shape = (C_tp.shape[0], 1))
        R_t           = tf.reshape(tf.reduce_mean(R_tp, axis = 1), shape = (C_tp.shape[0], 1))
        ...
        while index_delta_t< len(delta_list):       
            ...
            C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp_beta_2, R_tp_beta_1, R_tp = tf.map_fn(fn = step_t_multi, elems = index, dtype = (tf.float32, tf.int32, tf.float32, tf.float32, tf.float32, tf.float32))
            # C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp = tf.map_fn(fn = step_t_multi, elems = index, dtype = (tf.float32, tf.int32, tf.float32, tf.float32))

            ancestors = 1#tf.concat( (ancestors, tf.reshape(A_t,                           shape = (C_tp.shape[0], C_tp.shape[2], 1)) ), axis = 2)
            infected  = tf.concat( (infected,  tf.reshape(tf.reduce_mean(tf.reduce_sum(C_tp, axis = 1), axis = 1), shape = (C_tp.shape[0], 1, 1)) ), axis = 2)     

            loc_H_I_multi = lambda i: tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, C_tp[i,:,:])
            H_tp          = tf.map_fn(fn = loc_H_I_multi, elems = index, dtype = (tf.float32))
            households    = tf.concat( (households, tf.reshape(tf.reduce_mean(H_tp, axis = 2), shape = (H_tp.shape[0], H_tp.shape[1], 1))), axis = 2) 

            R_tp_beta_2 = tf.convert_to_tensor(np.nanmean(R_tp_beta_2.numpy(), axis = 1), dtype = tf.float32)
            R_tp_beta_1 = tf.convert_to_tensor(np.nanmean(R_tp_beta_1.numpy(), axis = 1), dtype = tf.float32)
            R_tp        = tf.convert_to_tensor(np.nanmean(R_tp.numpy(), axis = 1),        dtype = tf.float32) 

            # R_t_beta_2    = tf.concat( (R_t_beta_2, tf.reshape(R_tp_beta_2, shape = (C_tp.shape[0], H_tp.shape[1], 1))), axis = 2)
            # R_t_beta_1    = tf.concat( (R_t_beta_1, tf.reshape(R_tp_beta_1, shape = (C_tp.shape[0], H_tp.shape[1], 1))), axis = 2)
            # R_t           = tf.concat( (R_t,        tf.reshape(R_tp,        shape = (C_tp.shape[0], H_tp.shape[1], 1))), axis = 2)

            R_t_beta_2    = tf.concat( (R_t_beta_2, tf.reshape(R_tp_beta_2, shape = (C_tp.shape[0], 1))), axis = 1)
            R_t_beta_1    = tf.concat( (R_t_beta_1, tf.reshape(R_tp_beta_1, shape = (C_tp.shape[0], 1))), axis = 1)
            R_t           = tf.concat( (R_t,        tf.reshape(R_tp,        shape = (C_tp.shape[0], 1))), axis = 1)
        ...
        return ancestors, infected, households, R_t_beta_2, R_t_beta_1, R_t

    ################################################
    # Complete SMC to run during rejuvenation
    def filter_high_particles(self, Y, max_delta_t):
        ...
        # Run the step 0 of the SMC
        step_0_multi                          = lambda i: self.step_0(Y_t, self.theta[i,:])
        C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp_beta_1, R_tp = tf.map_fn(fn = step_0_multi, elems = index, dtype=(tf.float32, tf.int32, tf.float32, tf.float32, tf.float32))
        # Initialize the log-likelihood estimate
        # log_p_y_1_to_T = tf.reshape(log_p_y_t_given_y_1_to_tm1, shape = (log_p_y_t_given_y_1_to_tm1.shape[0], 1))
        ...
        R_t_beta_1    = tf.reshape(R_tp_beta_1, shape = (C_tp.shape[0], C_tp.shape[2], 1))
        R_t           = tf.reshape(R_tp,        shape = (C_tp.shape[0], C_tp.shape[2], 1))
        ...
        while index_delta_t< len(delta_list):       
            ...
            step_t_multi             = lambda i: self.step_t( t, self.theta[i,:], C_tp[i,:,:], A_t[i,:], Y_t, delta_t )
            C_tp, A_t, log_p_y_t_given_y_1_to_tm1, R_tp_beta_1, R_tp = tf.map_fn(fn = step_t_multi, elems = index, dtype = (tf.float32, tf.int32, tf.float32, tf.float32, tf.float32))
            ...
            R_t_beta_1    = tf.concat( (R_t_beta_1, tf.reshape(R_tp_beta_1, shape = (C_tp.shape[0], C_tp.shape[2], 1))), axis = 2)
            R_t           = tf.concat( (R_t, tf.reshape(R_tp,        shape = (C_tp.shape[0], C_tp.shape[2], 1))), axis = 2)

            # log_p_y_1_to_T = log_p_y_1_to_T + tf.reshape(log_p_y_t_given_y_1_to_tm1, shape = (log_p_y_t_given_y_1_to_tm1.shape[0], 1))
            index_delta_t = index_delta_t + 1

        return ancestors, infected, R_t_beta_1, R_t
