...
class proposal_epidemic():
    ...
    def __init__( self, households_individuals, 
        ...
        if self.epidemic_type=="2 parameters within-across households and fixed recovery":
            self.initial_colonize_prob = hyperparameters_epidemic["initial_colonize_prob"]
        ...
        if self.epidemic_type=="2 parameters within-across households and fixed recovery (sqrt)":
            self.initial_colonize_prob = hyperparameters_epidemic["initial_colonize_prob"]
        ...
        if self.epidemic_type=="2 parameters within-across households and fixed recovery, seasonality":
            self.initial_colonize_prob = hyperparameters_epidemic["initial_colonize_prob"]
    ...
    def proposal_epidemic_0(self, Y_0, theta):    
        ...
        if self.epidemic_type == "3 parameters within-across households and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            season = (1+self.coef*np.cos(self.periodicity*0+self.translation))

            # R_t
            healthy_individuals_household = (self.households_individuals.households_size-1)
            infection_k_kprime            = beta_1*(1)
            R_t_household_beta_1 = healthy_individuals_household*infection_k_kprime/(infection_k_kprime+self.gamma)
            R_t_beta_1           = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, R_t_household_beta_1, adjoint_a = True)

            distance_matrix                             = tf.exp( -tf.math.sqrt( self.distances/(phi*phi) ) )-tf.eye(self.distances.shape[0])
            distance_matrix_R_t  = beta_2*season*((1)/self.households_individuals.households_size)*distance_matrix/(beta_2*season*((1)/self.households_individuals.households_size)*distance_matrix + self.gamma)
            R_t_household_beta_2 = tf.tensordot(distance_matrix_R_t, self.households_individuals.households_size, axes = 1)
            R_t_beta_2           = tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, R_t_household_beta_2, adjoint_a = True)
        ...
        return C_0p, W_0p, tf.reduce_mean((R_t_beta_1+ R_t_beta_2)[(R_t_beta_1+ R_t_beta_2)!=0]), tf.reduce_mean(R_t_beta_1[R_t_beta_1!=0])
    ...
    def proposal_epidemic_t(self, t, theta, C_tp, Y_t, delta_t):
        ...
        if self.epidemic_type == "2 parameters within-across households and fixed recovery":
            beta_1          = tf.exp(theta[0])
            phi             = tf.exp(theta[1])
            individual_coef = tf.reshape(theta[2:-1], shape = (theta[2:-1].shape[0], 1) )
            coef            = theta[-1]
            ...
            distance_matrix                             = tf.exp(-self.distances/(2*phi*phi))-tf.eye(self.distances.shape[0])
            weighted_infection_per_household_prevalence = tf.tensordot( distance_matrix, infected_per_household/self.households_individuals.households_size, axes = 1)
            infection_rate_across_household             = beta_1*season*tf.sparse.sparse_dense_matmul( self.households_individuals.loc_H_I, 
                                                                                                       weighted_infection_per_household_prevalence, adjoint_a = True)
        ...
        if self.epidemic_type == "2 parameters within-across households and fixed recovery, seasonality":
            beta_1          = tf.exp(theta[0])
            phi             = tf.exp(theta[1])
            individual_coef = tf.reshape(theta[2:], shape = (theta[2:].shape[0], 1) )
            coef            = self.coef
        ...
        if self.epidemic_type == "3 parameters within-across households and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            # R_t
            healthy_individuals_household = (self.households_individuals.households_size-infected_per_household-1)
            infection_k_kprime            = beta_1*(infected_per_household+1)

            infection_kprime_from_same_household   = beta_1*(infected_per_household)
            infection_kprime_from_other_households = beta_2*season*tf.tensordot(distance_matrix, infected_per_household/self.households_individuals.households_size, axes = 1)

            R_t_household_beta_1 = healthy_individuals_household*infection_k_kprime/(infection_k_kprime+infection_kprime_from_same_household+infection_kprime_from_other_households+self.gamma)
            R_t_beta_1           = (1-C_tp)*tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, R_t_household_beta_1, adjoint_a = True)

            distance_matrix_R_t_k_kprime           = beta_2*season*((infected_per_household+1)/self.households_individuals.households_size)*distance_matrix
            distance_matrix_R_t  = distance_matrix_R_t_k_kprime/(distance_matrix_R_t_k_kprime + tf.transpose(infection_kprime_from_same_household) + tf.transpose(infection_kprime_from_other_households) + self.gamma)
            R_t_household_beta_2 = tf.tensordot(distance_matrix_R_t, self.households_individuals.households_size-infected_per_household, axes = 1)
            R_t_beta_2           = (1-C_tp)*tf.sparse.sparse_dense_matmul(self.households_individuals.loc_H_I, R_t_household_beta_2, adjoint_a = True)
        ...
        if self.epidemic_type == "3 parameters within-across households (all proportions) and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            # R_t
            healthy_individuals_household = (self.households_individuals.households_size-infected_per_household-1)
            infection_k_kprime            = beta_1*(infected_per_household+1)/self.households_individuals.households_size

            infection_kprime_from_same_household   = beta_1*(infected_per_household)/self.households_individuals.households_size
            infection_kprime_from_other_households = beta_2*season*tf.tensordot(distance_matrix, infected_per_household/self.households_individuals.households_size, axes = 1)
        ...
        if self.epidemic_type == "3 parameters within-across households (all proportions) and fixed recovery (sqrt)":
            ...
            phi             = tf.exp(theta[2])
            individual_coef = tf.reshape(theta[3:], shape = (theta[3:].shape[0], 1) )
            coef            = tf.reshape(theta[3:-1], shape = (theta[3:-1].shape[0], 1) )
        ...
        return C_tp1p, W_0p, tf.reduce_mean((R_t_beta_1+ R_t_beta_2)[(R_t_beta_1+ R_t_beta_2)!=0]), tf.reduce_mean(R_t_beta_1[R_t_beta_1!=0])