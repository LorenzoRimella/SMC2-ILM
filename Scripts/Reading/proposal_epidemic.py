...
class proposal_epidemic():
    ...
    ################################################
    # R_0
    def R_0_calculation_approx(self, t, theta):
        ...
        if self.epidemic_type == "3 parameters within-across households and fixed recovery":
            ...
            # compute how infected in household contribute to infection of each individual in the same household: L_{H,I}^T L_{H,I} C_tp
            infection_rate_from_household = beta_1*(self.households_individuals.households_size-1)

            # compute how other households contribute to infect individuals L_{H,I}^T D L_{H,I} C_tp 
            # (D should esclude the diagonal to avoid repeating the per household factor)
            distance_matrix                             = tf.exp(-self.distances/(2*phi*phi))-tf.eye(self.distances.shape[0])
            weighted_infection_per_household_prevalence = tf.tensordot( distance_matrix, self.households_individuals.households_size, axes = 1)
            infection_rate_across_household             = beta_2*season*weighted_infection_per_household_prevalence/self.households_individuals.households_size

            del distance_matrix, 
            del weighted_infection_per_household_prevalence, 
        ...
        if self.epidemic_type == "2 parameters within-across households and fixed recovery":
            ...
            distance_matrix                             = tf.exp(-self.distances/(2*phi*phi))-tf.eye(self.distances.shape[0])
            weighted_infection_per_household_prevalence = tf.tensordot( distance_matrix, self.households_individuals.households_size, axes = 1)
            infection_rate_across_household             = beta_1*season*weighted_infection_per_household_prevalence/self.households_individuals.households_size
        ...
        if self.epidemic_type == "3 parameters within-across households (all proportions) and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            # compute how infected in household contribute to infection of each individual in the same household: L_{H,I}^T L_{H,I} C_tp
            infection_rate_from_household = beta_1*(self.households_individuals.households_size-1)/self.households_individuals.households_size
        ...
        return tf.reduce_mean(infection_rate_from_household+infection_rate_across_household, axis = 0)/self.gamma, tf.reduce_mean(infection_rate_from_household, axis = 0)/self.gamma


    ################################################
    # R_0
    def R_0_calculation(self, t, theta):
        ...
        if self.epidemic_type == "3 parameters within-across households and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            # compute how infected in household contribute to infection of each individual in the same household: L_{H,I}^T L_{H,I} C_tp
            infection_rate_from_household = (self.households_individuals.households_size-1)*((beta_1)/(beta_1 + self.gamma))

            # compute how other households contribute to infect individuals L_{H,I}^T D L_{H,I} C_tp 
            # (D should esclude the diagonal to avoid repeating the per household factor)
            distance_matrix                             = tf.exp( -tf.math.sqrt( self.distances/(phi*phi) ) )-tf.eye(self.distances.shape[0])
            weighted_infection_per_household_prevalence = beta_2*season*tf.tensordot( distance_matrix, self.households_individuals.households_size, axes = 1)/self.households_individuals.households_size
            infection_rate_across_household             = (weighted_infection_per_household_prevalence)/(weighted_infection_per_household_prevalence+self.gamma)
        ...
        if self.epidemic_type == "3 parameters within-across households (all proportions) and fixed recovery, seasonality, individuals coefficients (sqrt)":
            ...
            # compute how infected in household contribute to infection of each individual in the same household: L_{H,I}^T L_{H,I} C_tp
            infection_rate_from_household = (self.households_individuals.households_size-1)*((beta_1/self.households_individuals.households_size)/(beta_1/self.households_individuals.households_size + self.gamma))
        ...
        return tf.reduce_mean(infection_rate_from_household+infection_rate_across_household, axis = 0), tf.reduce_mean(infection_rate_from_household)